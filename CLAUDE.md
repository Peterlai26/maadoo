...
- **Storage:** logged-in (non-guest) users sync to Supabase `profiles` (`plus_until`, `coins`, `free_month_claimed`), `coin_ledger` and `questions`, all RLS own-rows only. `coins` is never written by the client; the `coin_ledger` trigger maintains it and enforces the rules above. Guests and logged-out users are sent to log in / add an email before subscribing, asking or claiming coins. Without Supabase everything runs in memory.
